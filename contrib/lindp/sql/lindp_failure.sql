...
-- Low threshold so these small joins exercise LinDP; keep the search
-- deterministic.  The fallback triggers at every effort level, so the default
-- effort is fine.
SET lindp.enabled = true;
SET lindp.min_threshold = 3;
SET lindp.fallback = false;
SET geqo = off;
