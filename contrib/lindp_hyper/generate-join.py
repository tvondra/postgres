...
def plan_hyper(sql, seeds):
	...
	cur.execute("LOAD 'lindp_hyper'")
	cur.execute('set lindp_hyper.enabled = on')
	cur.execute('set lindp_hyper.fallback = off')
	cur.execute('set lindp_hyper.min_relations = 2')
	cur.execute('set lindp_hyper.max_relations = 100')
	cur.execute(f'set lindp_hyper.seeds = {seeds}')
