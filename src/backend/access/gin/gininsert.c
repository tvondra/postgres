...
AssertCheckGinBuffer(GinBuffer *buffer)
{
	 ...
	 *
	 * FIXME I believe it should always be sorted, so the flag does not seem
	 * to be necessary.
	 */
...
_gin_compare_tuples(GinTuple *a, GinTuple *b, SortSupport ssup)
{
	...
	if (a->category == GIN_CAT_NORM_KEY)
	{
		...
		return (r != 0) ? r : ItemPointerCompare(GinTupleGetFirst(a), GinTupleGetFirst(b));
	}

	return ItemPointerCompare(GinTupleGetFirst(a), GinTupleGetFirst(b));
}
