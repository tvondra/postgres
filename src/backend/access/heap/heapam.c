...
/* ----------------
 *		changeset_open - open a changeset relation by relation OID
 *
 *		If lockmode is not "NoLock", the specified kind of lock is
 ...
 *		index already.)
 *
 *		An error is raised if the changeset does not exist.
 *
 ...
 */
Relation
changeset_open(Oid relationId, LOCKMODE lockmode)
{
	Relation	r;

	r = relation_open(relationId, lockmode);

	if (r->rd_rel->relkind != RELKIND_CHANGESET)
		ereport(ERROR,
				(errcode(ERRCODE_WRONG_OBJECT_TYPE),
				 errmsg("\"%s\" is not a changeset",
						RelationGetRelationName(r))));
...
/* ----------------
 *		changeset_close - close a changeset relation
 *
 *		If lockmode is not "NoLock", we then release the specified lock.
 *
 ...
 */
void
changeset_close(Relation relation, LOCKMODE lockmode)
{
	LockRelId	relid = relation->rd_lockInfo.lockRelId;
