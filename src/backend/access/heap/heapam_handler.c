...
typedef struct ClusterShared
{
	...
	/* Incremented by each worker after joining / attaching. */
	int			last_worker_id;
								...
								Sharedsort *sharedsort, Relation heap, int sortmem)
{
	...
	/* print a couple tuplesort stats from this worker */
	{
		TuplesortInstrumentation stats;
		memset(&stats, 0, sizeof(TuplesortInstrumentation));
		tuplesort_get_stats(tuplesort, &stats);
		elog(WARNING, "scans %d  tuples %d  space used %ld", nscans, ntuples, stats.spaceUsed);
	}
