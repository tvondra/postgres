					  ...
					  void *per_buffer_data)
{
	 ...
	 * chance to return scanPos items and potentially end entirely (e.g., with
	 ...
	 * physical/logical correlation, where batches contain groups of adjacent
	...
	else if (!hscan->xs_yield_check)
	{
		...
		{
			/*
			 * Never yield during the initial ramp-up phase of the scan.
			 * Yielding too early prevents the read stream from building up an
