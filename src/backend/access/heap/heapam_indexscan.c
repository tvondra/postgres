								 ...
								 void *per_buffer_data)
{
		...
		index_scan_pos_cmp(prefetchPos, scanPos, xs_read_stream_dir) < 0)
	{
		...
		*prefetchPos = *scanPos;
		...
		if (scan->xs_want_itup)
		{
			...
			{
				HeapBatchData *hbatch = index_scan_batch_table_area(scan,
																	prefetchBatch);
	...
	for (;;)
	{
		...
		if (fromScanPos)
		{
			 ...
			 * the heap block from TID passed to heapam_index_fetch_tuple at
			 * the point where it called read_stream_next_buffer for the first
			 * time during the scan. (As explained above, we also end up here
			 * during the first call to read_stream_next_buffer following
