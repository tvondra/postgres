...
tableam_util_scanbatch_dirchange(IndexScanDesc scan)
{
	...
	while (index_scan_batch_count(scan) > 1)
	{
		...
		tableam_util_release_batch(scan, tail);
...
indexam_util_alloc_batch(IndexScanDesc scan)
{
	...
	if (!batch)
	{
		...
		/*
		 * Subtotal #1: the size of both fixed-size AM opaque areas located
		...
		if (scan->xs_want_itup)
		{
			 ...
			 */
			if (scan->xs_heapfetch->batch_per_item_size > 0)
				ios_table_trailing_sz =
			...
			/*
			 * An index-only scan will always need a currTuples space, however
			 */
			ios_total_trailing_sz += scan->batch_tuples_workspace;

			/*
			 * Subtotal #3: The combined size of both index-only scan trailing
			 * areas (ios_total_trailing_sz)
			 */
			pg_assume(ios_total_trailing_sz > 0);
			Assert(ios_total_trailing_sz == MAXALIGN(ios_total_trailing_sz));
		}

		Assert(scan->xs_want_itup || ios_total_trailing_sz == 0);
