			   ...
			   Relation *Irel, int nindexes, bool aggressive)
{
	...
	BlockNumber empty_pages,
				vacuumed_pages;
	double		num_tuples,		/* live tuples (reltuples estimate) */
				tups_vacuumed,	/* tuples cleaned up by vacuum */
				nkeep,			/* non-removable tuples */
				nunused;
...
#define FORCE_CHECK_PAGE() \
			 ...
			 offnum = OffsetNumberNext(offnum))
		{
			...
			/*
			 * The criteria for counting a tuple as live (for the purpose of
			 * reltuples estimation) in this block need to match what
			 * acquire_sample_rows() is doing, otherwise VACUUM and ANALYZE
			 * may produce wildly different values e.g. when there are many
			 * recently dead tuples.
			 *
			 * Compared to acquire_sample_rows() the logic here is simpler,
			 * as VACUUM can't run inside a transaction block, which makes
			 * some cases impossible (e.g. in-progress insert from the same
			 * transaction).
			 */
			switch (HeapTupleSatisfiesVacuum(&tuple, OldestXmin, buf))
			{
				...
				case HEAPTUPLE_LIVE:
					...
					/*
					 * Count as live for reltuples estimate. Not only is this
					 * natural, but it's also what acquire_sample_rows() does.
					 */
					num_tuples += 1;
				...
				case HEAPTUPLE_INSERT_IN_PROGRESS:
					 ...
					 * the transaction to update the counters at commit. This
					 * is what acquire_sample_rows() does, so be consistent.
					 */
					all_visible = false;
					break;
				case HEAPTUPLE_DELETE_IN_PROGRESS:
					...
					/*
					 * Count these rows as live. We expect the transaction to
					 * update the counters at commit, so be consistent with
	...
	/*
	 * XXX This used to differ, but thanks to unifying what live tuples
	 * mean in IndexBuildHeapScan, ANALYZE and VACUUM it seems natural
	 * to hand over the same value to indexes.
	 */
	vacrelstats->new_rel_tuples = vacrelstats->new_live_tuples;
