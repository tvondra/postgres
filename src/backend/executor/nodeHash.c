...
#define BLOOM_MIN_HASH_SIZE (4*1024*1024)	/* 4MB */
...
MultiExecPrivateHash(HashState *node)
{
	...
	/*
	 * If forced to build a bloom filter, do that now, but only when there's a
	 * single batch. For batched mode this must have happened elsewehere earlier.
	 */
	if (force_hashjoin_bloom && (hashtable->nbatch == 1))
	{
...
BloomFilterInitRows(double nrows, double error)
{
	...
	elog(LOG, "BloomFilterInitRows: %lu bits (%lu bytes), %d hashes", nbits, (nbits + 7) / 8, nhashes);
...
BloomFilterInitBytes(Size nbytes, double error)
{
	...
	elog(LOG, "BloomFilterInitBytes: %lu bits (%lu bytes), %d hashes", 8 * nbytes, nbytes, nhashes);
