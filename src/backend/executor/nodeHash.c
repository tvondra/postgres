...
ExecHashTableCreate(HashState *state)
{
	...
	if (state->want_bloom_filter)
	{
		...
		/*
		 * New filter instance; bump the generation so recipients reset their
		 * adaptive state instead of judging this fresh filter by the previous
		 * one's behavior (relevant when a rescan rebuilds the hashtable).
		 */
		state->bloom_filter_generation++;
