						...
						int *num_skew_mcvs)
{
	...
	*space_allowed = hash_table_bytes;
	 ...
	 *
	 * We only reduce the number of batches. We could try increasing nbatch
	 * too, but we don't want to force batching when it is not needed, even if
	 * that would reduce memory usage. In this case most of the memory is used
	 * by the in-memory hash table, and thus within the limit.
	 *
