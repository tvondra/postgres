						...
						int *num_skew_mcvs)
{
	...
	*space_allowed = hash_table_bytes;
	 ...
	 * table, ignoring the memory used by batch files. But each batch file has
	 * a BLCKSZ buffer, and we may need two files per batch (inner and outer
	 * side). So with enough batches this can be significantly more memory than
	 ...
	 *
	 * where (inner_rel_bytes / nbatch) is the size of the in-memory hash table
	 * and (2 * nbatch * BLCKSZ) is the amount of memory used by file buffers.
	 * But for sufficiently large values of inner_rel_bytes value there may not
	 * be a nbatch value that would make both parts fit into hash_table_bytes.
	 *
	 ...
	 *
	 * The formula for total memory usage says that given an inner relation
	 * of size inner_rel_bytes, we may divide it into an arbitrary number of
	 * of batches. This determines both the size of the in-memory hash table
	 * and the amount of memory needed for batch files. These two terms work
	 * in opposite ways - when one decreases, the other increases.
	 *
	 ...
	 * do that by using the u-shape. We calculate the memory usage with half
	 * the batches (i.e. nbatch/2), and if it's lower than the current memory
	 * usage we know it's better to use fewer batches. We repeat this until
	 * reducing the number of batches does not reduce the memory usage - we
	 * found the optimum.
	 *
	 * We're only considering to reduce the number of batches. We could try
	 * the opposite direction too, and in some cases it might reduce the
	 * memory usage. But this can happen only when the hash table is using
	 * most of the memory, and the hash table is what we used for the initial
	 * sizing, so we can't be exceeding the memory limit too much. Adding
	 * batches might save a bit of memory, but would result in spilling many
	 * more batch files, which does not seem like great trade off.
	 *
	 * While growing the hashtable, we also adjust the number of buckets, to
	 * not have more than one tuple per bucket. We can only do this during the
	 * initial sizing - once we start building the hash, nbucket is fixed.
	 */
	while (nbatch > 0)
	{
...
ExecHashIncreaseBatchSize(HashJoinTable hashtable)
{
	 ...
	 * increases the memory usage the least is the same as comparing the values
	 * directly.
	 */
