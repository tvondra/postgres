 ...
 * the bloom filter (a NULL never equals anything under a strict hash operator),
 * but we still let it pass through to the upstream join rather than reject it,
 * so the join (rather than us) decides what to do with it (e.g. emit
 * NULL-extended for an outer join).  That is always safe: the only tuples the
 * filter may legitimately drop are ones with no match on the build side, and a
 * NULL-keyed tuple would be dropped by the join anyway.
 */
static inline bool
					...
					uint32 *hashvalue)
{
	...
	if (isnull)
		return false;			/* NULL strict key: pass through (see above) */
...
ExecBloomFilters(List *filters, ExprContext *econtext)
{
	...
	foreach(lc, filters)
	{
		...
		/*
		 * If the filter was rebuilt since we last probed it, our adaptive
		 * state describes a different filter and would wrongly keep
		 * sampling/skipping a fresh, possibly selective filter. Start it over
		 * for the new filter generation.
		 */
		if (bfs->seen_generation != hashNode->bloom_filter_generation)
		{
			bfs->adaptiveSampling = false;
			bfs->adaptiveWindowProbes = 0;
			bfs->adaptiveWindowMatches = 0;
			bfs->adaptiveSampleCounter = 0;
			bfs->seen_generation = hashNode->bloom_filter_generation;
		}
