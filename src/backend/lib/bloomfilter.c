...
/*
 * Like bloom_create(), but the minimum size of the bitset is provided by the
 * caller (in bytes) instead of using the default minimum of 1MB.
 *
 * This is useful for callers that build many small Bloom filters, where a 1MB
 * floor would waste memory.  All other behavior matches bloom_create().
 */
bloom_filter *
					...
					Size min_filter_size, uint64 seed)
{
	...
	/* Allocate bloom filter as a single palloc'd chunk */
	filter = palloc(bloom_estimate_custom(total_elems, bloom_work_mem,
										  min_filter_size));

	return bloom_init_custom(filter, total_elems, bloom_work_mem,
							 min_filter_size, seed);
}
...
/*
 * Size of the filter's bitset, in bits.
 */
uint64
...
/*
 * Estimate the filter's current false positive rate.
 *
 * The probability that a non-member element is reported as possibly present is
 * approximately the proportion of bits that are set raised to the power of the
