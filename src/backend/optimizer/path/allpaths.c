							...
							RelOptInfo *rel, Relids build_relids)
{
	...
	{
		SimpleRelOptInfo *brel = (SimpleRelOptInfo *) lfirst(lc);

		if (bms_equal(brel->relids, relids))
		{
			nrows = brel->rows;
			break;
		}
...
find_interesting_bloom_filters(PlannerInfo *root, RelOptInfo *rel)
{
	...
	foreach(lc, build_sides)
	{
		...
		/*
		 * Cap the build side at bloom_filter_pushdown_max_build_relids.  The
		 * enumeration deliberately goes one relation further (so the join
		 * ratio for a max-sized build side can be looked up), but those extra
		 * sets exist only for that lookup and must not become filters of their
		 * own.
		 */
		if (bms_num_members(brel->relids) > bloom_filter_pushdown_max_build_relids)
			continue;
...
generate_useful_gather_paths(PlannerInfo *root, RelOptInfo *rel, bool override_rows)
{
	...
	foreach(lc, useful_pathkeys_list)
	{
		...
		foreach(lc2, rel->partial_pathlist)
		{
			...
			/*
			 * Skip paths with pushed-down filters (not supported across a
			 * Gather).  generate_gather_paths() applies the same guard; do it
			 * here too, otherwise a filter-bearing partial path could still be
			 * placed under a Gather Merge via the sort paths built below.
			 */
			if (subpath->expected_filters != NIL)
				continue;
