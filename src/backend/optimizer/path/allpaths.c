...
find_interesting_bloom_filters(PlannerInfo *root, RelOptInfo *rel)
{
	 ...
	 *    bloom_build_side_join_ratio), which accounts for restrictions on
	 *    build relations and the join clauses among them -- including build
	 *    relations with no direct join clause to the owner.
	 *
	...
	foreach(lc, build_sides)
	{
		...
		/*
		 * Have a clause to the filter "owner" (where we push the filter)
