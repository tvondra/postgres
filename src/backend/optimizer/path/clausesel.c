...
static bool clause_is_ext_compatible(Node *clause, Index relid, Bitmapset **attnums,
						int type);

static Bitmapset *collect_ext_attnums(List *clauses, Index relid, int type);

static int	count_ext_attnums(List *clauses, Index relid, int type);
...
static Selectivity clauselist_ext_selectivity(PlannerInfo *root,
						  List *clauses, StatisticExtInfo *stats);

static Selectivity clauselist_ext_selectivity_ndist(PlannerInfo *root,
...
static Selectivity clauselist_ext_selectivity_mcvlist(PlannerInfo *root,
								  List *clauses, StatisticExtInfo *stats,
								  bool *fullmatch, Selectivity *lowsel);

static Selectivity clauselist_ext_selectivity_histogram(PlannerInfo *root,
									List *clauses, StatisticExtInfo *stats);
 ...
 *
 * Finally we remove the portion of clauses estimated using extended stats,
					   ...
					   SpecialJoinInfo *sjinfo)
{
	...
	/* list of extended stats on the relation */
	List	   *stats = NIL;
	 ...
	 * the extended statistics anyway, but that requires a more expensive
						  ...
						  STATS_TYPE_MCV | STATS_TYPE_HIST) >= 2))
	{
									  ...
									  STATS_TYPE_MCV | STATS_TYPE_HIST)))
		{
			...
			clauses = clauselist_ext_split(root, relid, clauses, &mvclauses,
									stat, STATS_TYPE_MCV | STATS_TYPE_HIST);
	...
	if (has_stats(stats, STATS_TYPE_FDEPS) &&
		(count_ext_attnums(clauses, relid, STATS_TYPE_FDEPS) >= 2))
	{
		StatisticExtInfo *stat;
		Bitmapset  *mvattnums;
	...
	if (has_stats(stats, STATS_TYPE_NDIST) &&
		(count_ext_attnums(clauses, relid, STATS_TYPE_NDIST) >= 2))
	{
		StatisticExtInfo *stat;
		Bitmapset  *mvattnums;
 ...
 */
static Selectivity
clauselist_ext_selectivity(PlannerInfo *root, List *clauses, StatisticExtInfo *stats)
{
	...
	s1 = clauselist_ext_selectivity_mcvlist(root, clauses, stats,
										   &fullmatch, &mcv_low);
	...
	s2 = clauselist_ext_selectivity_histogram(root, clauses, stats);
...
static Selectivity
clauselist_ext_selectivity_ndist(PlannerInfo *root, Index relid,
								List *clauses, StatisticExtInfo *stats,
								...
								SpecialJoinInfo *sjinfo)
{
	...
	Assert(stats->ndist_enabled && stats->ndist_built);

	/* load the ndistinct items stored in the statistics */
	ndistinct = load_ext_ndistinct(stats->mvoid);

	/* collect attnums in the clauses */
	attnums = collect_ext_attnums(clauses, relid, STATS_TYPE_NDIST);
	...
	if (item)
	{
		...
		foreach(lc, clauses)
		{
			 ...
			 */
			if (! clause_is_ext_compatible(clause, relid, &attnums_clause,
										  STATS_TYPE_NDIST))
			 ...
			 * but clause_is_ext_compatible only accepts (Var=Const) clauses.
			 */
							   ...
							   SpecialJoinInfo *sjinfo)
{
	...
	dependencies = load_ext_dependencies(stats->mvoid);
	...
	while (true)
	{
		...
		attnums = collect_ext_attnums(clauses, relid, STATS_TYPE_FDEPS);
 ...
 */
static Bitmapset *
collect_ext_attnums(List *clauses, Index relid, int types)
{
	...
	foreach(l, clauses)
	{
		...
		clause_is_ext_compatible(clause, relid, &attnums, types);
	}
 ...
 */
static int
count_ext_attnums(List *clauses, Index relid, int type)
{
	int			c;
	Bitmapset  *attnums = collect_ext_attnums(clauses, relid, type);
					...
					StatisticExtInfo *stats, int types)
{
	...
	*mvclauses = NIL;
	...
	foreach(l, clauses)
	{
		...
		if (clause_is_ext_compatible(clause, relid, &attnums, types))
		{
 ...
 */
static bool
clause_is_ext_compatible(Node *clause, Index relid, Bitmapset **attnums, int types)
{
...
find_stats(PlannerInfo *root, Index relid)
{
	...
	return root->simple_rel_array[relid]->statlist;
}
 ...
 */
static Selectivity
clauselist_ext_selectivity_mcvlist(PlannerInfo *root, List *clauses,
								  StatisticExtInfo *stats, bool *fullmatch,
								  Selectivity *lowsel)
{
	int			i;
	Selectivity s = 0.0;
	...
	mcvlist = load_ext_mcvlist(stats->mvoid);
	...
	memset(matches, STATS_MATCH_FULL, sizeof(char) * mcvlist->nitems);
 ...
 */
static Selectivity
clauselist_ext_selectivity_histogram(PlannerInfo *root, List *clauses,
									 StatisticExtInfo *stats)
{
	...
	mvhist = load_ext_histogram(stats->mvoid);
