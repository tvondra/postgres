	...
	for (pass = 0; pass < 2; pass++)
	{
		...
		foreach(lc, filters)
		{
			...
			else if (bms_is_subset(f->build_relids, join_relids))
			{
				 ...
				 *
				 * Require the filter's build side to be exactly this join's
				 * other (build) side, not merely a subset of it.  The physical
				 * filter built at plan time uses all of the hash join's keys
				 * (createplan.c copies hj->hashkeys), so its actual selectivity
				 * reflects the entire build side.  If we realized a filter
				 * whose build_relids covered only part of that side, the row
				 * estimate -- credited from f->selectivity, computed over
				 * f->build_relids alone -- would diverge from the filter that
				 * actually runs.  We don't lose the filter by being strict: a
				 * candidate matching the whole build side is enumerated
				 * separately, and gets realized at the join whose build side it
				 * matches exactly.
				 */
				if (is_hashjoin &&
					owner_in_outer &&
					bms_equal(f->build_relids, other_relids) &&
						  ...
						  bool parallel_hash)
{
	 ...
	 */
	{
		...
		/*
		 * A parallel-aware hash join can't build a pushed-down Bloom filter
		 */
		bool		can_realize = !parallel_hash;

		jfilters = compute_join_expected_filters(root, outer_path, inner_path,
												 jointype, can_realize,
												 can_realize ? hashclauses : NIL,
												 &contradicted,
												 can_realize ? &realized : NULL);
