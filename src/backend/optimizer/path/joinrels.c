...
enumerate_bloom_filter_build_relids(PlannerInfo *root)
{
	...
	/*
	 * Join levels, indexed by number of relids (starts at 1 for baserels,
	 * same as in standard_join_search).
	 *
	 * We enumerate one level beyond bloom_filter_pushdown_max_build_relids:
	 * bloom_build_side_join_ratio() estimates a build side of B relations from
	 * the cardinality of the join {owner} + B, which has B+1 relations.  So to
	 * get a real estimate for a build side of up to max_build_relids relations
	 * we must have enumerated join relations of up to max_build_relids + 1.
	 */
	levels = palloc0_array(List *, (bloom_filter_pushdown_max_build_relids + 2));
	...
	/*
	 * Build higher levels by adding one base relation at a time.  We go one
	 * level beyond max_build_relids so a build side of exactly that many
	 * relations still gets a join-cardinality estimate (see the comment on the
	 * levels array above, and bloom_build_side_join_ratio).
	 */
	for (level = 2; level <= bloom_filter_pushdown_max_build_relids + 1; level++)
	{
		ListCell   *lc;
		...
		foreach(lc, levels[level - 1])
		{
			...
			foreach(lc2, levels[1])
			{
				 ...
				 * number of join relids (to keep the planning effort under
				 * control).
				 */
