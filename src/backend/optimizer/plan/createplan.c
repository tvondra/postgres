...
find_bloom_filter_recipient(Plan *plan, Index target_relid)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_Unique:
			{
				/*
				 * These nodes preserve their input rows, so pushing a filter
				 * to a scan below them would be valid in principle. In
				 * practice we never actually reach a recipient this way today
				 * (a Sort/Unique between the realizing join and the scan comes
				 * with a subquery boundary, and Material/Memoize land on the
				 * build side), which the Assert records. If that ever changes,
				 * the producer's rescan handling,the filter is freed and
				 * rebuilt together with its hashtable, see ExecReScanHashJoin,
				 * must be verified to work through these nodes before we rely
				 * on routing a filter past them.
				 */
				Plan	   *res = find_bloom_filter_recipient(outerPlan(plan),
															  target_relid);
		...
		case T_Limit:
			...
			/*
			 * Never push a filter below a LIMIT: dropping probe rows beneath it
			 * would change which rows the LIMIT returns.  (We never reach a
			 * recipient through a LIMIT today anyway, as for the nodes above,
			 * but unlike them this would be wrong rather than merely
			 * unhandled, so we stop here in all builds.)
			 */
			return NULL;
		case T_HashJoin:
		case T_NestLoop:
