...
static List *starjoin_join_order(PlannerInfo *root, List *item_list, List *joinlist);
...
deconstruct_jointree(PlannerInfo *root)
{
	...
	/*
	 * Now that we have all the quals and clauses distributed to rels, we can
	 * walk the joint tree and split some of the lists to force a particular
	 * shape of the join tree.
	 *
	 * XXX I believe we can't do this earlier, because we need to be able to
	 * decide what is a dimension, and that's done using the clauses (we have
	 * to determine if the join is on a foreign key and that the dimension
	 * has no other restrictions).
	 */
	result = starjoin_join_order(root, item_list, result);
 ...
 */
static bool
starjoin_match_to_foreign_key(PlannerInfo *root, List *item_list, RelOptInfo *rel)
{
	ListCell   *lc;
	...
	foreach(lc, root->fkey_list)
	{
		...
		for (int i = 0; i < fkinfo->nkeys; i++)
		{
			...
			AttrNumber	attnum = fkinfo->confkey[i];

			foreach(cell, item_list)
			{
				JoinTreeItem *jtitem = (JoinTreeItem *) lfirst(cell);

				if (IsA(jtitem->jtnode, JoinExpr))
				{
					ListCell *cell2;
					JoinExpr   *j = (JoinExpr *) jtitem->jtnode;

					/* XXX should do the same checks as match_foreign_keys_to_quals */
					foreach(cell2, (List *) j->quals)
					{
						Node *expr = (Node *) lfirst(cell2);
						Relids varnos = pull_varnos(root, expr);
						// OpExpr *opexpr = (OpExpr *) expr;
						Bitmapset *varattnos = NULL;

						/* only opexprs (can other expressions match FK?) */
						if (!IsA(expr, OpExpr))
							continue;

						/* not a member of the expr */
						if (!bms_is_member(rel->relid, varnos))
							continue;

						pull_varattnos(expr, rel->relid, &varattnos);

						/* expect only a single attnum */
						if (bms_num_members(varattnos) != 1)
							continue;

						if (bms_singleton_member(varattnos) + FirstLowInvalidHeapAttributeNumber != attnum)
							continue;
...
//			/*
//			 * try finding a matching restrictinfo
//			 *
//			 * XXX This does not work, because the joininfo is not built yet,
//			 * it only happens in standard_join_search/make_join_rel :-( And
//			 * it's on the joinrel, not the baserels anyway.
//			 */
...
//
//				/*
//				 * See if rinfo was previously matched to FK as a "loose" clause.
//				 *
//				 * FIXME has to consider the FK direction too, a dimension has to be
//				 * on the PK side of the constraint
//				 */
...
//			}
		...
		{
			// elog(WARNING, "starjoin_match_to_foreign_key true");
			return true;
		}
	}

	// elog(WARNING, "starjoin_match_to_foreign_key false");
 ...
 */
static bool
starjoin_is_dimension(PlannerInfo *root, List *item_list, RangeTblRef *rtr)
{
	RelOptInfo *rel;
	RangeTblEntry *rte;
	...
	/* XXX should check reloptkind != RELOPT_BASEREL ? */
	if (rte->rtekind != RTE_RELATION)
		return false;

	/* FIXME Is it correct to lookup reloptinfo by rtindex? */
	 ...
	 */
	if (!starjoin_match_to_foreign_key(root, item_list, rel))
		return false;

	// FIXME inspect joininfo
	...
	// elog(WARNING, "dimension %s", rte->eref->aliasname);
 ...
 */
static List *
starjoin_join_order(PlannerInfo *root, List *item_list, List *joinlist)
{
	ListCell *lc;
	List *newlist = NIL;
	List *dimensions = NIL;

	/* do nothing if starjoin ordering optimzation disabled */
	...
	foreach (lc, joinlist)
	{
		...
		if (IsA(item, List))
		{
			newlist = lappend(newlist,
							  starjoin_join_order(root, item_list, (List *) item));
			continue;
		}
		 ...
		 */
		Assert(IsA(item, RangeTblRef));	/* paranoia */
		 ...
		 */
		if (starjoin_is_dimension(root, item_list, (RangeTblRef *) item))
		{
	...
	/* if we found some dimensions, build the tree now */
	if (dimensions != NIL)
	{
