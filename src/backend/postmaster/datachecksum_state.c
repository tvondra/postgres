								 ...
								 int cost_limit)
{
	...
	if (!launcher_running)
	{
		...
		{
			ereport(LOG,
					errmsg("data checksums already in desired state"));
			return;
		}
...
again:
	...
	if (DataChecksumState->launch_operation != operation)
	{
		elog(LOG, "XXX: restarting datachecksums op: was: %s, new: %s",
			(operation == ENABLE_DATACHECKSUMS ? "enable" : "disable"),
			(DataChecksumState->launch_operation == ENABLE_DATACHECKSUMS ? "enable" : "disable"));
...
DataChecksumsWorkerMain(Datum arg)
{
	...
	foreach_oid(reloid, RelationList)
	{
		...
		{
			elog(LOG, "XXX: updating cost settings");
