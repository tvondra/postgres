						   ...
						   ReorderBufferTupleBuf *tuplebuf)
{
	...
	else
	{
		...
		PG_TRY();
		{
			 ...
			 *
			 * XXX It seems we can only see cases with (log_cnt = 0), as both
			 * nextval_internal() and do_setval() only write WAL records with
			 * log_cnt=0. But better to handle that case too.
			 *
			 * XXX Should this somehow consider is_called too?
			 */
			value = (seq->last_value + seq->log_cnt);
