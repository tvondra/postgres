...
/*
 * bms_member_index
 *		determine index of the varattno in the bitmap
 *
 * Returns (-1) when the varattno is not a member.
 */
int
bms_member_index(Bitmapset *keys, AttrNumber varattno)
{
	...
	Assert(AttrNumberIsForUserDefinedAttr(varattno));

	i = -1;
	j = 0;
	while (((i = bms_next_member(keys, i)) >= 0))
	{
		if (i == varattno)
			return j;

		/* exceeded the requested attnum / not a member */
		if (i > varattno)
			return -1;
