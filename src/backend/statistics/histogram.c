...
/*
 * builds a multivariate histogram
 *
 ...
 */
MVHistogram
build_ext_histogram(int numrows, HeapTuple *rows, int2vector *attrs,
...
/* fetch the histogram (as a bytea) from the pg_statistic_ext catalog */
MVSerializedHistogram
load_ext_histogram(Oid mvoid)
{
	bool		isnull = false;
	Datum		histogram;

#ifdef USE_ASSERT_CHECKING
	// Form_pg_statistic_ext mvstat;
#endif
...
#ifdef USE_ASSERT_CHECKING
	// mvstat = (Form_pg_ext_statistic) GETSTRUCT(htup);
	...
	return deserialize_ext_histogram(DatumGetByteaP(histogram));
}

/* print some basic info about the histogram */
Datum
pg_histogram_info(PG_FUNCTION_ARGS)
{
	bytea	   *data = PG_GETARG_BYTEA_P(0);
	char	   *result;

	MVSerializedHistogram hist = deserialize_ext_histogram(data);
 ...
 */
bytea *
serialize_ext_histogram(MVHistogram histogram, int2vector *attrs,
					   VacAttrStats **stats)
{
	int			i = 0,
				j = 0;
 ...
 */
MVSerializedHistogram
deserialize_ext_histogram(bytea *data)
{
	int			i = 0,
				j = 0;
	...
	Assert((nbuckets > 0) && (nbuckets <= STATS_HIST_MAX_BUCKETS));
 ...
 */
static MVBucket
create_initial_ext_bucket(int numrows, HeapTuple *rows, int2vector *attrs,
						 VacAttrStats **stats)
{
	int			i;
	int			numattrs = attrs->dim1;
...
pg_histogram_buckets(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		histogram = load_ext_histogram(mvoid);
...
debug_histogram_matches(MVSerializedHistogram mvhist, char *matches)
{
	...
	for (i = 0; i < mvhist->nbuckets; i++)
	{
		...
		nfull += (matches[i] == STATS_MATCH_FULL) ? 1 : 0;
		npartial += (matches[i] == STATS_MATCH_PARTIAL) ? 1 : 0;

		/* and also update the frequencies */
		ffull += (matches[i] == STATS_MATCH_FULL) ? bucket->ntuples : 0;
		fpartial += (matches[i] == STATS_MATCH_PARTIAL) ? bucket->ntuples : 0;
