 ...
 */
MCVList
build_ext_mcvlist(int numrows, HeapTuple *rows, int2vector *attrs,
...
load_ext_mcvlist(Oid mvoid)
{
	...
	return deserialize_ext_mcvlist(DatumGetByteaP(mcvlist));
}
...
pg_stats_ext_mcvlist_info(PG_FUNCTION_ARGS)
{
	...
	MCVList		mcvlist = deserialize_ext_mcvlist(data);
 ...
 */
bytea *
serialize_ext_mcvlist(MCVList mcvlist, int2vector *attrs,
					 VacAttrStats **stats)
{
	int			i,
				j;
 ...
 */
MCVList
deserialize_ext_mcvlist(bytea *data)
{
	int			i,
				j;
	Size		expected_size;
...
pg_stats_ext_mcvlist_items(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		mcvlist = load_ext_mcvlist(PG_GETARG_OID(0));
