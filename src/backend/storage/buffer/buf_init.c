...
buffer_partitions_prepare(void)
{
	...
	/* XXX can this happen? */
	Assert(numaRegistry->num_nodes >= 1);
...
/* FIXME make this work without NUMA */

Size
NUMAShmemSize(void)
{
	/* FIXME count only nodes with CPUs, and active CPUs */
	/* XXX only gives us the number, the nodes may not be 0, 1, 2, ... */
	num_nodes = numa_num_configured_nodes();

	num_cpus = get_nprocs();	/* XXX maybe use get_nprocs_conf() instead? */

	elog(LOG, "NUMA registry size %ld",
		 offsetof(NUMARegistry, nodes_cores) + sizeof(bool) * num_nodes * num_cpus);

	return offsetof(NUMARegistry, nodes_cores) + sizeof(bool) * num_nodes * num_cpus;
}
