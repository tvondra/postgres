...
buffer_partitions_init(void)
{
	 ...
	 * BufferGetNode().
	 *
	 * We need to account for partitions being of different length, when
	 * the NBuffers is not nicely divisible. To do that we keep track of
	...
	for (int i = 0; i < numa_partitions; i++)
	{
		...
		elog(LOG, "buffer_partitions_init: %d => %d buffers %d start %p end %p (size %ld)",
		...
		elog(LOG, "buffer_partitions_init: %d => %d descriptors %d start %p end %p (size %ld)",
			 i, part->numa_node, part->num_buffers, startptr, endptr, (endptr - startptr));
