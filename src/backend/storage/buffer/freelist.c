...
typedef struct BufferStrategyFreelist
{
	...
	/* XXX Not sure why this needs to be aligned like this. Need to ask Andres. */
	int			firstFreeBuffer	__attribute__((aligned(64)));	/* Head of list of unused buffers */
...
typedef struct
{
	 ...
	 *
	 * XXX Not sure why this needs to be aligned like this. Need to ask Andres.
	 * Also, shouldn't the alignment be specified after, like for "consumed"?
	 */
...
ChooseFreeList(void)
{
	...
	/*
	 * freelist is partitioned, so determine the CPU/NUMA node, and pick
	...
	/*
	 * FIXME This doesn't work well if CPUs are excluded from being run
	 * or offline. In that case we end up not using some freelists at all,
	 * but not sure if we need to worry about that. Probably not for now.
	 * But could that change while the system is running?
	 *
	 * XXX Maybe we should somehow detect changes to the list of CPUs,
	 * and rebuild the lists if that changes? But that seems expensive.
	 */
...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state, bool *from_ring)
{
	...
	*from_ring = false;
	 ...
	 *
	 * XXX Should we also make this NUMA-aware, to only access buffers from the
	 * same NUMA node? That'd probably mean we need to make the clock sweep
	 * NUMA-aware, perhaps by having multiple clock sweeps, each for a subset of
	 * buffers. But that also means each process could "sweep" only a fraction
	 * of buffers, even if the other buffers are better candidates for eviction.
	 * Would that also mean we'd have multiple bgwriters, one for each node, or
	 * would one bgwriter handle all of that?
	 */
	trycounter = NBuffers;
	for (;;)
	{
...
StrategyInitialize(bool init)
{
	...
	if (!found)
	{
		...
		/*
		 * Walk through the buffers, add them to the correct list. Walk from the
		 * end, because we're adding the buffers to the beginning.
		 */
