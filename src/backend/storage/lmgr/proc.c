...
PGProcShmemSize(void)
{
	 ...
	 * We can fit ~2k entries on a 2MB page, and NUMA does stuff with page
	 * granularity, and the large NUMA systems are likely to use huge pages. So
	 * with sensible max_connections we would not use more than a single page,
	 * which means it gets to a single NUMA node.
	 *
...
FastPathLockShmemSize(void)
{
	 ...
	 * NUMA node - but this way we add two pages per node - one for PGPROC, one
	 * for fast-path arrays. In theory we could make this work just one page per
	 * node, by adding fast-path arrays right after PGPROC entries on each node.
	 * But now we allocate fast-path locks separately - good enough for PoC.
	 *
	 * FIXME Should be conditional, but that was causing problems in bootstrap
	 * mode. Or maybe it was because the code that allocates stuff later does
	 * not do that conditionally. Anyway, needs to be fixed.
...
InitProcGlobal(void)
{
	...
	/*
	 * also build PGPROC entries for auxiliary procs / prepared xacts (we don't
	 * assign those to any NUMA node)
	 *
