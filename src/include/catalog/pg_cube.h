 ...
 * ----------------
 */
#define CubeRelationId  3350

CATALOG(pg_cube,3350) BKI_WITHOUT_OIDS BKI_SCHEMA_MACRO
{
	Oid			cubeid;			/* OID of the cube */
	Oid			cuberelid;		/* OID of the relation it aggregates */
	Oid			cubechsetid;	/* OID of the changeset used to feed it */
	int16		cubenatts;		/* number of columns in the cube */

	/* variable-length fields start here, but we allow direct access to cubekey */
	int2vector	cubekey;		/* column numbers of cube cols, or 0 */
