...
typedef struct BloomFilterState
{
	...
	/*
	 * Generation of the producer's filter we last probed. When it no longer
	 * matches HashState.bloom_filter_generation the filter has been rebuilt
	 * (e.g. by a rescan), so the adaptive state above is stale and gets
	 * reset.
	 */
	uint64		seen_generation;
} BloomFilterState;
...
typedef struct HashState
{
	...
	/*
	 * Bumped every time the bloom filter is (re)built in ExecHashTableCreate,
	 * so a recipient can tell that a rescan replaced the filter and reset its
	 * per-filter adaptive state (a rescan that reuses the hashtable does not
	 * rebuild the filter and leaves this unchanged).
	 */
	uint64		bloom_filter_generation;
